...
const steps = [
  ...
  { id: 4, title: 'Final', desc: 'Konfirmasi' }
]
...
const handleSubmit = async () => {
  ...
  if (success) {
    alert(formStore.successMessage);
    router.push('/success-page'); // Atau sesuaikan rute Anda
  } else {
    alert(formStore.errorMessage);
  }
}
...
<template>
  ...
  <div class="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 pt-40">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="relative flex justify-between">
          <div v-for="step in steps" :key="step.id" class="flex flex-col items-center">
            ...
            <div class="mt-2 text-center">
              <p class="text-xs font-bold text-gray-900 hidden sm:block">{{ step.title }}</p>
              <p class="text-[10px] text-gray-500 hidden sm:block">{{ step.desc }}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="p-8">
            ...
            <div class="space-y-4">
              <div class="space-y-1">
                ...
                <div class="relative">
                  ...
                  <input v-model="formData.nama_lengkap" type="text" placeholder="Masukkan nama sesuai ijazah" class="form-input" />
                </div>
              </div>
              <div class="space-y-1">
                ...
                <div class="relative">
                  ...
                  <textarea v-model="formData.alamat" rows="3" placeholder="Jl. Contoh No. 123, Kota..." class="form-input !pl-10"></textarea>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="space-y-4">
              <div class="space-y-1">
                ...
                <div class="relative">
                  ...
                  <input v-model="formData.nama_orang_tua" type="text" placeholder="Nama wali murid" class="form-input" />
                </div>
              </div>
              <div class="space-y-1">
                ...
                <div class="relative">
                  ...
                  <input v-model="formData.nomor_orang_tua" type="tel" placeholder="08123456789" class="form-input" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="md:col-span-2 space-y-1">
                ...
                <div class="relative">
                  ...
                  <input v-model="formData.email" type="email" placeholder="contoh@mail.com" class="form-input" />
                </div>
              </div>
              <div class="space-y-1">
                <label class="text-sm font-semibold text-gray-700">Username</label>
                <input v-model="formData.username" type="text" placeholder="santri_2025" class="form-input" />
              </div>
              <div class="space-y-1">
                <label class="text-sm font-semibold text-gray-700">Password</label>
                <div class="relative">
                  <Lock class="absolute left-3 top-3 w-5 h-5 text-gray-400" />
                  <input v-model="formData.password" type="password" placeholder="••••••••" class="form-input" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="currentStep === 4" class="space-y-6">
            ...
            <div class="bg-gray-50 rounded-xl p-6 space-y-4 text-sm">
              <div class="grid grid-cols-2 border-b border-gray-200 pb-2">
                <span class="text-gray-500">Nama Santri</span>
                <span class="font-bold text-gray-900">{{ formData.nama_lengkap }}</span>
              </div>
              <div class="grid grid-cols-2 border-b border-gray-200 pb-2">
                <span class="text-gray-500">Nama Wali</span>
                <span class="font-bold text-gray-900">{{ formData.nama_orang_tua }}</span>
              </div>
              <div class="grid grid-cols-2 border-b border-gray-200 pb-2">
                <span class="text-gray-500">Username</span>
                <span class="font-bold text-primary-600">{{ formData.username }}</span>
              </div>
              <div class="grid grid-cols-1">
                <span class="text-gray-500 mb-1">Alamat</span>
                <span class="font-medium text-gray-800">{{ formData.alamat }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="mt-10 flex justify-between items-center border-t border-gray-100 pt-6">
            ...
            <button
                ...
                class="flex items-center gap-2 px-8 py-2.5 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 disabled:opacity-50 transition"
            >
      ...
      <p class="mt-8 text-center text-gray-400 text-xs">&copy; 2025 SantriConnect - Sistem PSB Pesantren Modern</p>
    </div>
  </div>
</template>
